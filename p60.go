...
func problem51() string {
    const max, targetFamilySize = 1000 * 1000, 8
    pfCount := make(map[int]int) // prime-family map: pfid -> count
    pfMin := make(map[int]int)   // prime-family map: pfid -> min

    toPrimeFamilyID := func(digits, group []int, groupExclude int) int {
        lenG := uint(len(group))
        id := 0 // a base-11 number

        gi := uint(0)
        for i, d := range digits {
            id *= 11

            if gi < lenG && i == group[gi] { // assumes group is sorted (asc)
                gm := 1 << gi
                gi++

                if (groupExclude & gm) > 0 {
                    id += 10 // stands in for the '*' char
                    continue
                }
            }
    ...
    // returns whether targetFamilySize is reached
    addToFamily := func(p, pf int) bool {
        count, exists := pfCount[pf]
        if !exists {
            count = 0
            pfMin[pf] = p
        }
        ...
        return count == targetFamilySize
    }

    addToAllFamilies := func(p int) (bool, int) {

        // build groups of repeated digits
        dg := make([][]int, 10) // digit groups
        ...
        for _, g := range dg {
            ...
            smax := 1 << lenG
            for s := 1; s < smax; s++ {
                pf := toPrimeFamilyID(digits, g, s)
                if addToFamily(p, pf) {
                    return true, pfMin[pf]
                }
            }
        }
    ...
    for p := 2; p < max; p++ {
        if !isComposite.Get(p) {
            if found, result := addToAllFamilies(p); found {
                return itoa(result)
            }
    ...
    panic("No solution found. Increase max")
}
...
func (c Cards) Less(i, j int) bool {
    return c[i].rank < c[j].rank || (c[i].rank == c[j].rank && c[i].suit < c[j].suit)
}
...
func problem54() string {
    ...
    scoreHand := func(hand []Card) (int, int) {
        ...
        if b, s := isRoyalFlush(hand); b {
            return score, s
        }
        score--

        if b, s := isStraight(hand); b {
            return score, s
        }
        score--

        if b, s := isFourOfAKind(hand); b {
            return score, s
        }
        score--

        if b, s := isFullHouse(hand); b {
            return score, s
        }
        score--

        if b, s := isFlush(hand); b {
            return score, s
        }
        score--
        ...
        if b, s := isThreeOfAKind(hand); b {
            return score, s
        }
        score--

        if b, s := isTwoPairs(hand); b {
            return score, s
        }
        score--

        if b, s := isOnePair(hand); b {
            return score, s
        }
        score--
...
func problem57() string {
    ...
    for i := 1; i < 1000; i++ {
        ...
        if hasMoreDigits(numer, denom, powOf10) {
            count++
        }
    }
    return itoa(count)
...
func problem59() string {
    ...
    xorDecode := func(a, b, c int) (bool, int) {
        ...
        for i := range encoded {
            ...
            if dec < asciiMin || dec > asciiMax {
                return false, -1
            }
            if (i == 0) && (dec != 32) && (dec != 34) && (dec != 39) && (dec != 40) && (dec < 65 || dec > 122) {
                return false, -1
            }
            if dec == asciiSpace {
                spaceCount++
                sinceLastSpace := i - lastSpace
                if sinceLastSpace > 30 {
                    return false, -1
                }
                lastSpace = i
            }
